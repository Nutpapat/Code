...
import pygal
def worldpop():
    total_csv = pd.read_csv("Total population.csv", encoding = "UTF-8")
    country = input()
    step = int(input())
    pop_list = list()
    years_list = list()
    ...
    if hav:
        ...
        line_chart.x_labels = [str(i) for i in range(1960, 2018, step)]
        line_chart.value_formatter = lambda x: "{:,} People".format(x)
        line_chart.add('Populations', pop_list)
        line_chart.render_to_file('pop_bar_chart.svg')  
        print(*pop_list)
    else:
        print('not Found')
worldpop()
