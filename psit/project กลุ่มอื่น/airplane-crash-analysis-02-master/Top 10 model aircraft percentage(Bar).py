...
, 'Fokker F-27', 'Piper PA-31', 'de Havilland DHC-2']
    ...
    sale = [844, 3871, 2500, 607, 1280, 586, 2500, 1267, 1398, 976]
    percent = []
    for i in range(10):
        ans = (int(list_val[i])/int(sale[i]))*100
        percent.append(int(ans))
    print(percent)
    line_chart = pygal.Bar()
    line_chart.title = 'Show the ratio between number of bulit and crash rate'
    line_chart.x_labels = list_lable
    line_chart.add('Percent :', percent)
    line_chart.render_to_file('Percent_Type_of_airplane.svg')
recheck()
